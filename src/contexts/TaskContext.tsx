...
export const TaskProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const updateTask = async (id: string, updatedTaskData: Partial<Omit<Task, "id" | "createdAt">>) => {
        ...
        try {
            ...
            if (updatedTaskData.estimationType !== undefined)
                apiTaskData.estimation_type = updatedTaskData.estimationType
            if (updatedTaskData.estimationValue !== undefined)
                apiTaskData.estimation_value = updatedTaskData.estimationValue
            ...
            setTasks((prev) => prev.map((task) => (task.id === id ? { ...task, ...updatedTaskData } : task)))

            // Update stats
            const updatedTasks = tasks.map((task) => (task.id === id ? { ...task, ...updatedTaskData } : task))
